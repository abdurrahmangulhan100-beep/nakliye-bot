...
http.createServer((req, res) => {
  ...
  if (pairingCode) {
    res.end(`
      ...
      <html>
        <head>
          <title>WhatsApp Bağlantı Kodu</title>
          ...
          <style>
            ...
            .code { font-size: 32px; font-weight: bold; letter-spacing: 4px; color: #075e54; background: #e7fce8; padding: 15px 25px; border-radius: 12px; margin: 20px 0; border: 2px dashed #25d366; }
            h2 { color: #128c7e; margin-bottom: 10px; }
            ol { text-align: left; color: #444; line-height: 1.6; }
          </style>
        </head>
        <body>
          <div class="card">
            <h2>🔑 Nakliye Cepte Bağlantı Kodu</h2>
            <div class="code">${pairingCode}</div>
            <ol>
              <li>Telefonundan <b>WhatsApp</b>'ı aç.</li>
              <li><b>Bağlı Cihazlar -> Cihaz Bağla</b> seçeneğine tıkla.</li>
              <li>En alttaki <b>"Telefon numarası ile bağla"</b> seçeneğini seç.</li>
              <li>Ekranda gördüğün 8 haneli kodu yukarıdaki gibi gir!</li>
            </ol>
          </div>
        </body>
      </html>
    ...
    res.end(`
      ...
      <html>
        <body style="font-family:sans-serif; text-align:center; padding-top:50px;">
          <h2> Nakliye Cepte WhatsApp Botu Aktif</h2>
          <p>Kod üretiliyorsa lütfen sayfayı 5 saniye sonra yenileyin.</p>
        </body>
      </html>
    `);
  }
...
const PHONE_NUMBER = process.env.PHONE_NUMBER; // Örn: 905xxxxxxxxx

const supabase = createClient(SUPABASE_URL, SUPABASE_KEY);

// --- 1.5 SUPABASE JET HIZINDA OTURUM YÖNETİCİSİ (IN-MEMORY CACHE) ---
...
async function botuBaslat() {
  ...
  // EĞER CİHAZ KAYITLI DEĞİLSE TELEFON NUMARASI İLE PAIRING CODE İSTE
  if (!sock.authState.creds.registered && PHONE_NUMBER) {
    setTimeout(async () => {
      try {
        const temizNumara = PHONE_NUMBER.replace(/[^0-9]/g, '');
        const code = await sock.requestPairingCode(temizNumara);
        pairingCode = code;
        console.log('\n==================================================');
        console.log(`🔑 WHATSAPP EŞLEŞTİRME KODUNUZ: ${code}`);
        console.log(`🔗 VEYA SİTEDEN BAKIN: https://nakliye-bot.onrender.com/`);
        console.log('==================================================\n');
      } catch (err) {
        console.error('Pairing Code Alma Hatası:', err.message);
      }
    }, 4000);
  }
