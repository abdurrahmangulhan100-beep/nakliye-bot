...
// --- QR KOD WEB SUNUCUSU ---
let qrDataURL = null; // QR Kod resim linkini tutacak değişken

const PORT = process.env.PORT || 8080;
http.createServer((req, res) => {
  if (req.url === '/qr' && qrDataURL) {
    // Tarayıcıda tam kare HD QR Kod gösterir
    ...
    res.end(`
      ...
      <html>
        ...
        <body>
          <div class="card">
            <h2>Nakliye Cepte - WhatsApp Bağlantısı</h2>
            <img src="${qrDataURL}" alt="WhatsApp QR Code" />
            <p>Lütfen WhatsApp -> Bağlı Cihazlar kısmından taratın.</p>
          </div>
        </body>
      </html>
    `);
...
async function botuBaslat() {
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (qr) {
      ...
      console.log('\n==================================================');
      console.log('👉 QR KODU TAM KARE GÖRMEK İÇİN BU LİNKE TIKLAYIN:');
      console.log(`🔗 https://nakliye-bot.onrender.com/qr`);
      console.log('==================================================\n');
    }
    
    if (connection === 'open') {
      qrDataURL = null; // Bağlanınca QR kod linkini temizle
