      ...
      <html>
        ...
        <body>
          <div class="card">
            ...
            <div class="badge">⚡ Jet Hızında Bağlantı Modu Aktif</div><br>
    ...
    res.end(`
      ...
      <html>
        <body style="font-family:sans-serif; text-align:center; padding-top:50px; background:#f0f2f5;">
          ...
          <p>Lütfen sayfayı 3 saniye sonra yenileyin veya bekleyin.</p>
...
// --- 3. SIFIR GECİKMELİ (NON-BLOCKING) SUPABASE SESSION MİMARİSİ ---
async function useSupabaseAuthState(supabase, sessionId = 'default_session') {
  const memoryCache = new Map();

  // Başlangıçta veritabanındaki verileri RAM'e al
  try {
    ...
    console.error('Session yükleme hatası:', err.message);
  }

  // RAM'e ANINDA yaz, Supabase'e arka planda async kaydet (Zaman aşımını önleyen anahtar nokta!)
  const writeData = (id, data) => {
    if (data) {
      memoryCache.set(id, data);
      const value = JSON.stringify(data, BufferJSON.replacer);
      supabase.from('session').upsert({ id, data: value }).then(() => {}).catch(() => {});
    } else {
      memoryCache.delete(id);
      supabase.from('session').delete().eq('id', id).then(() => {}).catch(() => {});
    }
  };
  ...
  const creds = readData(`creds_${sessionId}`) || initAuthCreds();
  ...
  return {
    state: {
      ...
      keys: {
        get: async (type, ids) => {
          const data = {};
          for (const id of ids) {
            let value = readData(`${type}_${id}_${sessionId}`);
    ...
    saveCreds: async () => {
      writeData(`creds_${sessionId}`, creds);
    }
  };
}
...
function mukerrerMesajMi(mesajMetni) {
  const temiz = mesajMetni.trim().toLowerCase();
  const simdi = Date.now();
  if (mesajHafizasi.has(temiz) && (simdi - mesajHafizasi.get(temiz) < 15 * 60 * 1000)) return true;
  mesajHafizasi.set(temiz, simdi);
  return false;
}
...
async function botuBaslat() {
  ...
  const sock = makeWASocket({
    ...
    syncFullHistory: false, // Eski sohbet geçmişini indirmeyi engelleyerek kilitlenmeyi çözer
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (qr) {
      ...
      console.log('👉 Taze QR Kod üretildi: /qr sayfasını kontrol edin.');
    }
    
    if (connection === 'open') {
      qrDataURL = null;
      console.log('\n==================================================');
      console.log('✅ WHATSAPP BOTU CANLI İLANLARI DİNLİYOR!');
      console.log('==================================================\n');
    }
    ...
    if (connection === 'close') {
      ...
      if (shouldReconnect) {
        ...
        console.log('❌ Oturum kapatıldı. Lütfen Supabase session tablosunu temizleyip yeniden başlatın.');
      }
    }
  });
