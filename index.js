...
// --- 1.5 SUPABASE KALICI OTURUM YÖNETİCİSİ (KALICI AUTH) ---
...
async function botuBaslat() {
  ...
  const sock = makeWASocket({
    version,
    auth: state,
    printQRInTerminal: false, // qrcode-terminal ile özel basacağımız için kapattık
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (qr) {
      console.log('\n--- KÜÇÜK QR KODU AŞAĞIDADIR, TARATIN ---');
    ...
    if (connection === 'close') {
      ...
      if (shouldReconnect) {
        ...
        console.log('❌ WhatsApp Oturumu Kapatıldı. Yeniden QR Okutmanız Gerekebilir.');
      }
    }
  });
